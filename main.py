...
def create_pull_request(github_client, issue, branch_name):
    pr = github_client.create_pull_request("main", branch_name, body=f"This is the result of fixit for issue {issue.number}", title=f"Fixit for issue {issue.number}")
...
def main():
    ...
    pr = create_pull_request(github_client, issue, branch_name)
